...
Requirements
    ...
    Conda:
        We use conda to install packages needed to run GUS-ID. If you have not already done so, please install miniconda >= 4.10.3 via:
        https://docs.conda.io/en/latest/miniconda.html

Setup

    The GUS-ID script has been packaged with a .yml file to build its conda environment. See below for instructions specific to osx-arm64.

    If not using osx-arm64 architecture, build the GUS-ID environment:
    ...
    Build the GUS-ID environment:
    conda env create -n GUS-ID_v2 --file create_GUS-ID_v2_env_arm64.yml

    Activate the conda environment:
    conda activate GUS-ID_v2

    Install BLASTp (2.12.0) to the GUS-ID_v2 by running the following command in the directory containing the downloaded package, replacing file name as appropriate:
...
Usage
    ...
    Prepare Directory.
        Place FASTA file into script directory alongside GUS-ID_v2.py.

    Run Script.
        To run GUS-ID_v2:
        python GUS-ID_v2.py {in_fasta.fasta}

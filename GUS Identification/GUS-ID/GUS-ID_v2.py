...
# BLASTp alignment threshold specified manually to 25% identity.
...
# create seed protein dictionary.
...
############################## ID GUS Functions and Definitions ##############################
...
# Generate log for runtimes and progress reporting.
                
# Generate logfile.
logging.basicConfig(filename='logfile.txt', level=logging.INFO, 
                    format='%(asctime)s:%(levelname)s:%(message)s', filemode='w')
header_message = f"GUS-ID_v2 - logfile for identifying GUS proteins in {db_fasta_file_str}"
...
# Concatenate results, clean directory, and finalize logfile.
...
# Log script runtime.
logging.info(f"Alignment started at: {start_time:.2f}, ended at: {end_time:.2f}, duration: {end_time - start_time:.2f} seconds.")

# Clean directory.
shutil.rmtree(alignment_root_out_path)

# Print output prompt to terminal.
print(f"GUS-ID run completed. Concatenated all GUS postive matches into {cat_GUS_out_name}. For each identified GUS, per-residue matches to best reference sequence are now located in {residues_root_out_path}.")
